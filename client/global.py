    ...
    def search(
        ...
        if query == "*" and filter_by:
            if query_by == "*" and self.first_string_field:
                ...
                self.logger.info(f"optimizing query: changing query_by to safe string field '{query_by}' for match-all filter.")

            if not sort_by and self.default_sorting_field:
                sort_by = f"{self.default_sorting_field}:asc"
                self.logger.info(f"optimizing query: setting sort_by to '{sort_by}' to disable relevancy ranking.")
...
def main():
    ...
    parser.add_argument("--per_page", type=int, help="Doc count per page(search)")
